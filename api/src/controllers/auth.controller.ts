...
export async function login(req: any, res: any) {
    ...
    try {
        var response = await axios.post('https://auth.etna-alternance.net/identity', {
            login,
            password
        });
    ...
    const token: string = response.headers['set-cookie'][0].replace("authenticator=", "");
...
export async function checkAuth(req: any, res: any) {
